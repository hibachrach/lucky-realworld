...
describe Api::Follows::Create do
  context "when not following the user" do
    it "follows the user and responds with serialized profile info" do
      ...
      response = client.post(path: Api::Follows::Create.with(username: user.username).url)
      ...
      FollowQuery.new.follower_id(user.id).followed_id(user.id).select_count.should eq(1)
    end
  end
end
